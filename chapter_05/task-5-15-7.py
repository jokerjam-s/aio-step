"""
Система модерации чат-комнаты (простая).

Написать асинхронный код, который будет модерировать список сообщений.
...
async def check_message(message: dict):
    for banned in banned_words:
        if f"{banned} " in message["message"].lower().replace('.', ' ').replace(',',' '):
            asyncio.current_task().cancel()
            print(f"В сообщении {message["message_id"]} стоп-слово: task.done(): {asyncio.current_task().done()}")
            break
    else:
...
async def main():
    tasks = [asyncio.create_task(check_message(m)) for m in messages]
