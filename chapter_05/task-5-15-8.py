"""
Система модерации чат-комнаты с учётом ролей
"""
import asyncio
        ...
        for banned in banned_words:
            if banned in message:
                if role == 'student':
                    ...
                    print(f"{role}: {message.replace(banned, '****')}")
                break
        else:
            print(f"{role}: {message}")
...
async def main():
    tasks = [asyncio.create_task(check_message(m["message"]), name=m['role']) for m in messages]
