"""
Проверка допуска новобранцев для секретной операции
"""
data = [
    {
        "Имя": "Sarah",
...
async def check_access(data: dict):
    ...
    if data.get("Срок доступа") is None:
        raise ValueError(
            f'Ошибка доступа: У участника {data["Имя"]} {data["Фамилия"]} срок доступа истек или не указан.')
    return f'Участник {data["Имя"]} {data["Фамилия"]} имеет действующий доступ. Продолжительность доступа: {data["Срок доступа"]}'


async def main():
    tasks = [asyncio.create_task(check_access(d), name=f"{d["Имя"]} {d["Фамилия"]}") for d in data]
